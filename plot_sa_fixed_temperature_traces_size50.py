"""Plot energy vs Metropolis swaps for fixed-temperature SA (size 50).

Reads ``sa_fixed_temp_traces_size50.csv`` and generates plots with
...
INPUT_CSV = Path("sa_fixed_temp_traces_size50.csv")
...
def load_traces():
    if not INPUT_CSV.is_file():
        raise SystemExit(
            ...
            "Run sa_fixed_temperature_experiment_size50.py first."
        )
...
def plot_energy_vs_swaps(traces):
    ...
    plt.title(f"Fixed-T SA size 50 (instance={inst}): energy vs swaps, steps_max={max_steps}")
...
def plot_bad_parameters(traces):
    ...
    plt.title(f"Fixed-T SA size 50 (instance={inst}): examples of bad parameters")
    plt.xlabel("Metropolis swap index t")
    plt.ylabel("Mean energy E(t)")
    plt.legend()
    plt.grid(True, alpha=0.3)
...
def main() -> None:
    ...
    fig1_path = OUTPUT_DIR / "sa_fixedT_energy_vs_swaps_size50.png"
    fig2_path = OUTPUT_DIR / "sa_fixedT_bad_parameters_size50.png"
    ...
    print("Saved fixed-temperature SA plots for size 50:")
    print(f" - {fig1_path}")
    print(f" - {fig2_path}")
